	...
	public class OctoMap 
	{
        ...
        private Vector3? GetRayIntersectionRecursive(ref Ray ray, float currentNodeSize, Vector3 currentNodeCentre, uint currentNodeId)
        {
            ...
            if (currentNode.ChildArrayId != null)
            {
                for (int i = 0; i < 8; i++)
                {
                    ...
                    Vector3? intersectedNodeCentre = GetRayIntersectionRecursive(ref ray, newNodeSize, newNodeCentre, childId);
        ...
        public void AddPoint(Vector3 point)
        {
            ...
            {            
                ...
                if (growCount > 20)
                {
                    Debug.Log("Aborted add operation as it seemed to be going on forever (" + (growCount - 1) + ") attempts at growing the OctoMap.");
                    return;
                }
            } 
        }
		...
		/// <param name="currentNodeCentre">The node centre the current recursive traversal is on.</param>
		/// <param name="currentNodeId">The node ID the current recursive traversal is on.</param>
        private void AddRayToOctoMap(Ray ray, Vector3 recordedPoint, float currentNodeSize, Vector3 currentNodeCentre, uint currentNodeId)
        {
            OctoMapNode currentNode = _nodes[currentNodeId];
		...
		/// <param name="currentNodeCentre">The node centre the current recursive traversal is on.</param>
		/// <param name="currentNodeId">The node ID the current recursive traversal is on.</param>
		private void GatherOctoMapNodes(List<OctoMapNodeRaw> nodes, float currentNodeSize, Vector3 currentNodeCentre, uint currentNodeId)
		{
			OctoMapNode currentNode = _nodes[currentNodeId];
			if (currentNode.Occupied >= 1)
			{
